(() => {
  const VERSION = '3.3.4';
  const BUTTON_NAME = '星月私立高等学院 控制中心 v3.3.4';
  ...
  const GIT_RUNTIME_REVISION = '3.3.4-statusbar-var-root-20260707';
  ...
  async function rerollCurrentVariables() {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.3.4 的当前楼变量重算器，只输出一个 <UpdateVariable> 块，不生成正文。' },
  ...
  async function previewVariableCorrection(instruction) {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.3.4 的变量定点修正器，只输出一个最小 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  async function rerollFromAnalysis(analysisText, floorId) {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.3.4 的当前楼变量重算器，依据玩家给定的变量预分析与正文，只输出一个 <UpdateVariable> 块，不生成正文、不输出 analysis。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  async function repairVariableFormatViaLLM() {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.3.4 的变量格式修复器，只输出一个最小 <UpdateVariable> 块，只修格式不改语义。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  const HUD_RT_BASE = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.3.4';
  const HUD_RT_BASE_CF = 'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.3.4';
  let hudPanel = null;
  let hudBlobUrl = null;
  ...
  async function generateNpcPerspective(targetName) {
    ...
    const result = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.3.4 的楼层内临时旁观视角生成器。结果只供玩家娱乐阅读，不进入后续上下文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  // 3.3.4:制造/NPC 面板 UI 已删,结算链在 07_craft,TA 浮窗在 12_npc_view
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      ...
      if (action === 'export-opening-draft') {
        downloadJson('xingyue-opening-draft-v3.3.4.json', readDraft());
      }
