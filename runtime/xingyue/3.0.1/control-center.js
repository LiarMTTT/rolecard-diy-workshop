(() => {
  const VERSION = '3.0.1';
  const BUTTON_NAME = '星月私立高等学院 控制中心 v3.0.1';
  ...
  function broadcastHudSettingsUpdate() {
    ...
    [window, hostWindow()].forEach(target => {
      ...
      try { target.dispatchEvent(new CustomEvent('xingyue-hud-settings-updated', { detail })); } catch (_) {}
    });
  }
  ...
  function readOpeningDraft() {
    try { return JSON.parse(localStorage.getItem('xingyue-opening-draft-v291') || '{}') || {}; } catch (_) { return {}; }
  }
  function writeOpeningDraft(patch) {
    const next = { ...readOpeningDraft(), ...(patch || {}) };
    try { localStorage.setItem('xingyue-opening-draft-v291', JSON.stringify(next)); } catch (_) {}
    return next;
  }
  function importPackageToDraft(pkg) {
    ...
    try { localStorage.setItem('xingyue-opening-draft-v291', JSON.stringify(draft)); } catch (_) {}
    return draft;
  }
  ...
  async function rerollCurrentVariables() {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.0.1 的当前楼变量重算器，只输出一个 <UpdateVariable> 块，不生成正文。' },
  ...
  async function previewVariableCorrection(instruction) {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.0.1 的变量定点修正器，只输出一个最小 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  async function rerollFromAnalysis(analysisText, floorId) {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.0.1 的当前楼变量重算器，依据玩家给定的变量预分析与正文，只输出一个 <UpdateVariable> 块，不生成正文、不输出 analysis。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  async function repairVariableFormatViaLLM() {
    ...
    const raw = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.0.1 的变量格式修复器，只输出一个最小 <UpdateVariable> 块，只修格式不改语义。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  async function generateNpcPerspective(targetName) {
    ...
    const result = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.0.1 的楼层内临时旁观视角生成器。结果只供玩家娱乐阅读，不进入后续上下文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  function toast(kind, message) { try { if (window.toastr?.[kind]) window.toastr[kind](message); } catch (_) {} }
  ...
  function renderAvatar(draft = readDraft()) {
    ...
    const src = avatar.startsWith('http') || avatar.startsWith('data:image') ? avatar : '';
    preview.innerHTML = src ? '<img src="' + escapeHtml(src) + '" alt="">' : escapeHtml('媒体库：' + avatar);
  ...
  function renderConfirmRail(draft) {
    ...
    const worldFactorCount = (draft.selected_world_factors || []).length + String(draft.custom_world_factor || '').split(/\r?\n/).map(s => s.trim()).filter(Boolean).length;
    ...
    try {
      ...
      if (action === 'login-discord') {
        if (state.workshopAuth && state.workshopAuth.loggedIn) {
          ...
          loginDiscord();
          setTimeout(() => refreshWorkshopAuth().catch(() => updateWorkshopStatusPills()), 1200);
        }
      }
      ...
      if (action === 'export-opening-draft') {
        downloadJson('xingyue-opening-draft-v3.0.1.json', readDraft());
      }
  ...
  const XY_OPENING_REMOTE_URL = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.0.1/opening-page.html';
  function loadRemoteOpeningPages(doc) {
    try {
      doc.querySelectorAll?.('[data-xy-opening-remote]:not([data-xy-remote-state])').forEach(mount => {
        ...
        fetch(XY_OPENING_REMOTE_URL, { cache: 'default' })
          ...
          .catch(e => {
            ...
            try { const l = mount.querySelector('[data-xy-opening-loading]') || mount; l.textContent = '开局界面远程加载失败（' + (e && e.message || e) + '）。请检查网络后刷新本楼层重试。'; } catch (_) {}
          });
      });
    } catch (_) {}
  }
