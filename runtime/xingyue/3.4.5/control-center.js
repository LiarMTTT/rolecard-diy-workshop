(() => {
  const VERSION = '3.4.5';
  const BUTTON_NAME = '星月私立高等学院 控制中心 v3.4.5';
  // 任务3.3：单一真相源 RUNTIME_BASE_URL；media_library.js/status_bar_regex.html 从 window.XY_RT_BASE 读（降级保留内联硬编码）
  const RUNTIME_BASE_URL = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5';
  ...
  const GIT_RUNTIME_REVISION = '3.4.5-stability-r35-20260713';
  ...
  function validatePackage(pkg, allowedTypes) {
    ...
    if (openingByScope) {
      ...
      if (!['3.4.1', '3.4.2', '3.4.3', '3.4.4', '3.4.5'].includes(pkg.payload.compatibility?.minRuntimeVersion)) throw new Error('开局正文包最低 runtime 必须为 3.4.1、3.4.2、3.4.3、3.4.4 或 3.4.5');
  ...
  const WORLDBOOK_FACTORY_BASELINE_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5/worldbook-factory-baseline.json?v=p9-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.5/worldbook-factory-baseline.json?v=p9-r1',
  ];
  const WORLDBOOK_FACTORY_BASELINE_SHA256 = '77f3e3dbd41d3f06815609577f01994336d64e244927e71e1eab6f596c8e27ff';
    ...
    worldbookFactoryBaselinePromise = (async () => {
      ...
      for (const url of WORLDBOOK_FACTORY_BASELINE_URLS) {
        try {
          ...
          if (data?.schema !== 'worldbook-factory-baseline-v1' || data?.cardVersion !== VERSION || data?.sha256 !== WORLDBOOK_FACTORY_BASELINE_SHA256 || data?.counts?.core !== 34 || data?.counts?.variable !== 5 || data?.counts?.total !== 39) throw new Error('3.4.5 出厂基线契约或固定 SHA-256 不匹配');
          return data;
        } catch (error) { lastError = error; }
      }
      throw lastError || new Error('无法加载 3.4.5 世界书出厂基线');
  ...
  const WORLDBOOK_EDITOR_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5/worldbook-editor.js?v=p7-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.5/worldbook-editor.js?v=p7-r1',
  ];
  ...
  async function previewApplyOpeningPackage(pkg) {
    ...
    if (pkg.cardScope !== OPENING_PACKAGE_SCOPE || pkg.payload?.target !== OPENING_PACKAGE_TARGET) throw new Error('该包不是 3.4.5 开局正文模板');
    const inspected = await inspectWorkshopPackage(pkg);
  ...
  async function rerollCurrentVariables() {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.5 的当前楼变量重算器，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
  ...
  async function previewVariableCorrection(instruction, floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.5 的变量定点修正器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function rerollFromAnalysis(analysisText, floorId) {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.5 的当前楼变量重算器，依据玩家给定的变量预分析与正文，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function repairVariableFormatViaLLM(floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.5 的变量格式修复器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，只修格式不改语义。' },
        { role: 'user', content: prompt },
      ],
  ...
  // 3.4.5：拖尾灵动感调优——tailRatio 0.58→0.70（更多粒子参与甩尾）、dragShear 1.28→1.72（甩尾幅度更明显）
  const XY_ORB_CFG = { n: 46, linkDist: 33, speed: 0.3, glowIntensity: 1.08, tailRatio: 0.70, dragShear: 1.72, colorTokens: { particle: '#6bc7f2', bright: '#4be4ff', highlight: '#cdf3ff' }, radius: 0 };
  const XY_ORB_STATES = {
  ...
  class PetOrbRenderer {
    ...
    nudge(dx, dy) {
      ...
      // 3.4.5：甩尾更明显——位移上限 0.9→1.15r 让尾巴甩得更远，速度踢 0.035→0.052 起手更活
    ...
    _update(dt) {
      ...
      const dtFrac = dt / 16;
      // 3.4.5：松手能量衰减 0.86→0.90，尾巴归位更绵长不戛然而止（更灵动）
      ...
      for (const p of this._particles) {
        ...
        if (p.tail && (p.sx || p.sy || p.svx || p.svy)) {
          // ⑧c 欠阻尼弹簧：拉回中心偏移。3.4.5 软化弹回——松手刚度 x2.8→x1.7 不再猛缩，
  ...
  function calcRadialPositions(center, vw, vh) {
    ...
    const rArc = 122;    // 3.4.5：5 键沿弧分布，半径从 108 加大到 122 避免相邻键堆叠（≥28/|cos75°| 仍满足不与球重叠）
  ...
  function ensureHudAvatarMgrStyle(doc) {
    ...
    style.textContent = [
      '#xingyue-hud-avatar-mgr{position:fixed;inset:0;z-index:2147483600;display:grid;place-items:center;background:rgba(4,8,12,.62);backdrop-filter:blur(3px);padding:16px;box-sizing:border-box;font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif}',
      '#xingyue-hud-avatar-mgr .xy-ham-panel{width:min(460px,100%);max-height:min(80vh,640px);overflow:auto;background:linear-gradient(180deg,#141d29,#0c1119);border:1px solid rgba(75,228,255,.28);border-radius:14px;box-shadow:0 24px 70px rgba(0,0,0,.55);color:#eafaff;padding:16px;box-sizing:border-box}',
  ...
  function openSidebarMenu() {
    ...
    menu.addEventListener('click', (event) => {
      ...
      try {
        ...
        if (act === 'avatar') openHudAvatarManager(); // 3.4.5：轮盘第5键——聊天上下文气泡头像管理器
  ...
  const HUD_RT_BASE = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5';
  const HUD_RT_BASE_CF = 'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5';
  let hudPanel = null;
  let hudDrawer = null;
  const hudSession = {
    phase: 'idle',
  ...
  async function generateNpcPerspective(targetName) {
    ...
    const result = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.5 的楼层内临时旁观视角生成器。结果只供玩家娱乐阅读，不进入后续上下文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  function renderOpeningDayEditor(draft = readDraft()) {
    ...
    try {
      ...
      if (statusNode) {
        ...
        statusNode.textContent = '正文有效 · ' + validation.bytes + ' bytes · ' + (dirty ? '已按当前聊天保存修改' : '正在使用 3.4.5 出厂版');
      }
  ...
  function openingDraftAsPackage() {
    ...
    const pkg = {
      ...
      summary:'由 3.4.5 入学日正文编辑器生成的纯文本模板。', authorName:'未署名',
  ...
  function setStep(next, options = {}) {
    ...
    root.querySelectorAll('.xy-step').forEach(button => {
      ...
      // 3.4.5：走过的步骤给完成态标记（✓ 表示访问过，不代表填写完整）
  ...
  function renderEnableLists() {
    ...
    root.querySelectorAll('[data-xy-enabled-preview]').forEach(node => {
      ...
      // 3.4.5：空态标薄样式（data-xy-empty），避免大块虚线空框
      if (items.length) {
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      ...
      if (action === 'export-opening-draft') {
        downloadJson('xingyue-opening-draft-v3.4.5.json', readDraft());
      }
      ...
      if (action === 'restore-opening-source') {
        ...
        if (source) {
          ...
          if (!confirm('当前正文没有工坊来源。确认恢复 3.4.5 官方出厂版？')) return;
      ...
      if (action === 'restore-opening-official') {
        if (!confirm('确认恢复 3.4.5 出厂正文？当前聊天里的正文修改将被替换。')) return;
        restoreOfficialOpeningDay();
        applyDraftToFields();
        renderWizard();
        toast('success', '已恢复 3.4.5 出厂正文');
      }
  ...
  const OPENING_PAGE_REVISION = '20260713-345-stability-r35';
  const OPENING_PAGE_SHA256 = '9ff08c5fd8d17722dfa30dd12d8e82e83e3f4d8c323190b3132f236dd7be413b';
  const OPENING_PAGE_SOURCES = [
    RUNTIME_BASE_URL + '/opening-page.html?v=' + OPENING_PAGE_REVISION,
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.5/opening-page.html?v=' + OPENING_PAGE_REVISION,
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.5/opening-page.html?v=' + OPENING_PAGE_REVISION,
  ];
  ...
  function sanitizeOpeningPageHtml(html, doc) {
    ...
    const root = parsed.querySelector('[data-xy-opening-page="3.4.5"]');
    if (!root
  ...
  function startOpeningRemoteLoad(mount, options = {}) {
    ...
    const loadedPage = mount.querySelector?.('[data-xy-opening-page="3.4.5"]')
