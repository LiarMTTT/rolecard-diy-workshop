(() => {
  const VERSION = '3.4.6';
  const BUTTON_NAME = '星月私立高等学院 控制中心 v3.4.6';
  // 任务3.3：单一真相源 RUNTIME_BASE_URL；media_library.js/status_bar_regex.html 从 window.XY_RT_BASE 读（降级保留内联硬编码）
  const RUNTIME_BASE_URL = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6';
  ...
  function validatePackage(pkg, allowedTypes) {
    ...
    if (openingByScope) {
      ...
      if (!['3.4.1', '3.4.2', '3.4.3', '3.4.4', '3.4.5', '3.4.6'].includes(pkg.payload.compatibility?.minRuntimeVersion)) throw new Error('开局正文包最低 runtime 必须为 3.4.1 至 3.4.6');
  ...
  const WORLDBOOK_FACTORY_BASELINE_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6/worldbook-factory-baseline.json?v=p9-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.6/worldbook-factory-baseline.json?v=p9-r1',
  ];
    ...
    worldbookFactoryBaselinePromise = (async () => {
      ...
      for (const url of WORLDBOOK_FACTORY_BASELINE_URLS) {
        try {
          ...
          if (data?.schema !== 'worldbook-factory-baseline-v1' || data?.cardVersion !== VERSION || data?.sha256 !== WORLDBOOK_FACTORY_BASELINE_SHA256 || data?.counts?.core !== 34 || data?.counts?.variable !== 5 || data?.counts?.total !== 39) throw new Error('3.4.6 出厂基线契约或固定 SHA-256 不匹配');
          return data;
        } catch (error) { lastError = error; }
      }
      throw lastError || new Error('无法加载 3.4.6 世界书出厂基线');
  ...
  const WORLDBOOK_EDITOR_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6/worldbook-editor.js?v=p7-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.6/worldbook-editor.js?v=p7-r1',
  ];
  ...
  async function previewApplyOpeningPackage(pkg) {
    ...
    if (pkg.cardScope !== OPENING_PACKAGE_SCOPE || pkg.payload?.target !== OPENING_PACKAGE_TARGET) throw new Error('该包不是 3.4.6 开局正文模板');
    const inspected = await inspectWorkshopPackage(pkg);
  ...
  async function rerollCurrentVariables() {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.6 的当前楼变量重算器，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
  ...
  async function previewVariableCorrection(instruction, floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.6 的变量定点修正器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function rerollFromAnalysis(analysisText, floorId) {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.6 的当前楼变量重算器，依据玩家给定的变量预分析与正文，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function repairVariableFormatViaLLM(floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.6 的变量格式修复器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，只修格式不改语义。' },
        { role: 'user', content: prompt },
      ],
  ...
  class PetOrbRenderer {
    ...
    _update(dt) {
      ...
      const dtFrac = dt / 16;
      // 3.4.6：松手能量衰减 0.86→0.90，尾巴归位更绵长不戛然而止（更灵动）
    ...
    _draw() {
      ...
      // 3.4.6：绘制缩放系数改按粒子场半径（drawScale=2*r/56，等于旧 canvas.width/56 的值），
  ...
  function calcRadialPositions(center, vw, vh) {
    ...
    const rArc = 122;    // 3.4.6：5 键沿弧分布，半径从 108 加大到 122 避免相邻键堆叠（≥28/|cos75°| 仍满足不与球重叠）
  ...
  function ensureHudAvatarMgrStyle(doc) {
    ...
    style.textContent = [
      // 3.4.6：overlay 用 left/top:0 + JS 显式设视口宽高（inset:0 在 ST 顶层 transform 下会塌成 32px 高→panel 跑屏外）；panel 用 calc(100%-…) 相对 overlay 硬边界防溢出
  ...
  // 3.4.6：HUD 段自含采集/缩略图，不跨闭包依赖开局段的 collectDialogSpeakers/avatarManagerThumb（那些闭包 root，HUD 段不可达会 ReferenceError）。
  ...
  // 3.4.6：HUD 段可达的 CC api getter——controlCenter() 在 bindOpeningPage 闭包内、HUD 段调它会 ReferenceError。
  ...
  // 3.4.6：显式把 overlay 尺寸钉到真实视口（绕 ST 顶层 transform 对 fixed inset:0 的高度塌缩劫持）
  ...
  function openHudAvatarManager() {
    ...
    sizeHudAvatarMgrOverlay(); // 3.4.6：钉视口尺寸→panel 正中且不超边界
    const win = hostWindow();
  ...
  function openSidebarMenu() {
    ...
    menu.addEventListener('click', (event) => {
      ...
      try {
        ...
        if (act === 'avatar') openHudAvatarManager(); // 3.4.6：轮盘第5键——聊天上下文气泡头像管理器
  ...
  const HUD_RT_BASE = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6';
  const HUD_RT_BASE_CF = 'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6';
  let hudPanel = null;
  let hudDrawer = null;
  const hudSession = {
    phase: 'idle',
  ...
  async function generateNpcPerspective(targetName) {
    ...
    const result = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.6 的楼层内临时旁观视角生成器。结果只供玩家娱乐阅读，不进入后续上下文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  function renderOpeningDayEditor(draft = readDraft()) {
    ...
    try {
      ...
      if (statusNode) {
        ...
        statusNode.textContent = '正文有效 · ' + validation.bytes + ' bytes · ' + (dirty ? '已按当前聊天保存修改' : '正在使用 3.4.6 出厂版');
      }
  ...
  function openingDraftAsPackage() {
    ...
    const pkg = {
      ...
      summary:'由 3.4.6 入学日正文编辑器生成的纯文本模板。', authorName:'未署名',
  ...
  function setStep(next, options = {}) {
    ...
    root.querySelectorAll('.xy-step').forEach(button => {
      ...
      // 3.4.6：走过的步骤给完成态标记（✓ 表示访问过，不代表填写完整）
  ...
  function renderEnableLists() {
    ...
    root.querySelectorAll('[data-xy-enabled-preview]').forEach(node => {
      ...
      // 3.4.6：空态标薄样式（data-xy-empty），避免大块虚线空框
      if (items.length) {
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      ...
      if (action === 'export-opening-draft') {
        downloadJson('xingyue-opening-draft-v3.4.6.json', readDraft());
      }
      ...
      if (action === 'restore-opening-source') {
        ...
        if (source) {
          ...
          if (!confirm('当前正文没有工坊来源。确认恢复 3.4.6 官方出厂版？')) return;
      ...
      if (action === 'restore-opening-official') {
        if (!confirm('确认恢复 3.4.6 出厂正文？当前聊天里的正文修改将被替换。')) return;
        restoreOfficialOpeningDay();
        applyDraftToFields();
        renderWizard();
        toast('success', '已恢复 3.4.6 出厂正文');
      }
  ...
  const OPENING_PAGE_REVISION = '20260713-346-stability-r36';
  const OPENING_PAGE_SHA256 = '4e301a073635af00073bf4f87d9f6a3b81c38ca5a758378adb7289ad5158a0eb';
  const OPENING_PAGE_SOURCES = [
    RUNTIME_BASE_URL + '/opening-page.html?v=' + OPENING_PAGE_REVISION,
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.4.6/opening-page.html?v=' + OPENING_PAGE_REVISION,
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.4.6/opening-page.html?v=' + OPENING_PAGE_REVISION,
  ];
  ...
  function sanitizeOpeningPageHtml(html, doc) {
    ...
    const root = parsed.querySelector('[data-xy-opening-page="3.4.6"]');
    if (!root
  ...
  function startOpeningRemoteLoad(mount, options = {}) {
    ...
    const loadedPage = mount.querySelector?.('[data-xy-opening-page="3.4.6"]')
