...
function buildWorldbookAiRequest(task, entry, context = {}) {
  ...
  return {
    ...
    orderedPrompts:[
      { role:'system', content:`你是星月 3.4.0 世界书编辑助手。${taskRules[task]} 仅输出符合 JSON Schema 的对象；结果只会进入草稿，不得声称已经保存。` },
      'user_input',
    ],
...
`;
  ...
  async function reload({ preserveDraft = false, force = false } = {}) {
    ...
    if (kept && entries.some(entry => entry.uid === kept.uid)) {
      original = clone(entries.find(entry => entry.uid === kept.uid)); draft = kept; selectedUid = kept.uid; conflictDraft = kept;
      setStatus('已重载最新原文并保留本地草稿；请重新检查 diff。','error');
  ...
  function close() {
    if (!root) return;
    cancelActiveAi('',false); closeModal(true); root.hidden = true;
