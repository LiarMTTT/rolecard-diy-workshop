(() => {
  const VERSION = '3.9.6';
  const BUTTON_NAME = '星月私立高等学院 控制中心 v3.4.9';
  // 任务3.3：单一真相源 RUNTIME_BASE_URL；media_library.js/status_bar_regex.html 从 window.XY_RT_BASE 读（降级保留内联硬编码）
  const RUNTIME_BASE_URL = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6';
  ...
  // 3.4.9 功能拓展：莉莉丝商店 3 条常驻内容（15/16/17 号世界书条目原文）搬进本 tab 预设，源条目 disable，勾选后合并写入下方动态条目（镜像世界因子机制）
  const EXTENSION_COMMENT = '[功能拓展]当前设定';
  const EXTENSION_PRESET_KEYS = Object.freeze(['技能区', '功能区', '道具蓝图']);
  const EXTENSION_PRESETS = Object.freeze({
    '技能区': `<shop_skills>
- 按摩术：Lv1 100分 / Lv5 5000分（解锁性感按摩、穴位催情）
- 开锁术：Lv1 80分 / Lv5 4000分（机械锁→电子锁→指纹锁）
- 潜行术：Lv1 100分 / Lv5 5000分（脚步消音→视觉盲区→气息隐匿）
- 道具制作：Lv1 200分 / Lv5 8000分
- 化学知识：Lv1 150分（识别药物→合成催情剂→特殊配方）
- 心理学：Lv1 200分（识破谎言→话术诱导→PUA技巧）
- 格斗术：Lv1 200分
- 摄影偷拍：Lv1 100分
</shop_skills>`,
    '功能区': `<shop_functions>
- 透视升级：5km→10km→20km（2000/8000/30000分）
- 扫描分辨率提升：1000/5000/20000分
- 催眠等级提升：Lv2 1000 / Lv3 5000 / Lv4 30000
- 时间暂停：实验功能 50000分
- 录像存储：HUD所见即录 500分解锁
- 通讯破解：手机/电脑入侵 2000分
- 监控接管：校园摄像头接入 5000分
- 信号屏蔽：制造无信号区 3000分
- 幻手：无法观测无法感知的无形之手，由{{user}}意念操控也可以交给莉莉丝代理操控，可穿透实体远程控制（大概500m），一次性最多可以使用4只幻手，可以发挥出与{{user}}相同的力量作用于物体，作用于人体时被作用者只会觉得是自己身体的反应（因为无法观测），每只幻手都能反馈触感给{{user}}
</shop_functions>`,
    '道具蓝图': `<shop_blueprints>
- 微型摄像头：500分/枚
- 定位追踪贴：300分/枚
- 催情香水（无色无味）：800分/瓶
- 安眠喷雾：600分/瓶
...
- 隐形耳麦：400分
- 制作要求：需对应技能等级解锁
- 纯欲斗篷套装：白色双排扣斗篷风衣 最长不超过大腿中部，只搭配白色丝袜和白色厚底高跟乐福鞋 不穿内衣，丝袜可搭配白丝连裤袜或白丝开档连裤袜 或过膝袜形成绝对领域
- 防走光肛塞：小巧易插入的肛塞，主动屏蔽{{user}}以外的人和摄影设备对裙底的窥伺，无法发现裙底的秘密 即便精液长长一条挂在小穴外也无法发现
- 静音跳蛋：一个不会发出任何声音的跳蛋，搭配遥控器可以远程控制
- 淫纹绘制器：可以在人体上附着带有发光效果（动情时互相发光）的生物适应性颜料（仅被绘制者直接和{{user}}可见），可通过再次使用设备移除 对人体无损；拥有两种模式，模式1是绘制阴道测量尺会扫描阴道的默认深度绘制在小腹上，模式2是淫纹模式 会根据角色的特点生成淫纹绘制在小腹上
</shop_blueprints>`,
  });
  ...
  // 3.7.5 #4 只锚角色 → 3.9.6 r64 #4 泛化为 {type,id}：世界因子/功能拓展浮窗发布此前每次按标题现算 id，
  ...
  const GIT_RUNTIME_REVISION = '3.9.6-stability-r69-20260718';
  ...
  function validatePackage(pkg, allowedTypes) {
    ...
    if (openingByScope) {
      ...
      if (!['3.4.1', '3.4.2', '3.4.3', '3.4.4', '3.4.5', '3.4.9', '3.5.0', '3.7.5', '3.9.0', '3.9.5', '3.9.6'].includes(pkg.payload.compatibility?.minRuntimeVersion)) throw new Error('开局正文包最低 runtime 必须为 3.4.1 至 3.9.6');
  ...
  // 3.9.6 r64 #3 总监裁定（2026-07-17）**显式推翻**会话级限制：登录 token 本就以 30 天期存 localStorage，
  ...
  // r65（Bug3 根治）：部分设备（隐私模式 / WebView 限制 / 存储被禁）localStorage.setItem 会抛错，旧实现 try-catch 静默吞掉
  // → token 从未存上 → /me 裸奔必 401 → 卡片报「登录 token 无效」并清态，重登也永远救不回（死循环）。
  // 修：内存副本为本会话第一真相源（必可用），localStorage 尽力持久化；持久化失败要如实告知「仅本次会话有效」。
  let workshopTokenMemory = null; // null=本会话尚未写过（读 storage）；string（含空串）=以内存为准
  let workshopTokenStorageBroken = false;
  function workshopTokenStores() {
    const stores = [];
    try { if (localStorage) stores.push(localStorage); } catch (_) {}
    try { const host = hostWindow(); if (host?.localStorage && stores.indexOf(host.localStorage) < 0) stores.push(host.localStorage); } catch (_) {}
    return stores;
  }
  function getWorkshopToken() {
    if (workshopTokenMemory !== null) return workshopTokenMemory;
    for (const store of workshopTokenStores()) {
      try { const value = store.getItem('xingyue-workshop-token'); if (value) return value; } catch (_) {}
    }
    return '';
  }
  function setWorkshopToken(token) {
    const value = String(token || '');
    workshopTokenMemory = value;
    let persisted = false;
    for (const store of workshopTokenStores()) {
      try {
        if (value) store.setItem('xingyue-workshop-token', value); else store.removeItem('xingyue-workshop-token');
        persisted = true;
      } catch (_) {}
    }
    workshopTokenStorageBroken = Boolean(value) && !persisted;
  }
  ...
  // 3.9.6 r64 #3：署名可见可改（工坊页署名输入框写这里）；清空=主动匿名发布——
  ...
  async function checkWorkshopAuth() {
    ...
    try {
      ...
      // 3.7.5 #4 / 3.9.6 r64 #3：已登录但内存无 identity（页面刷新/跨会话回来）→ 从持久化存储补回显示名+头像，供发布署名与身份 pill
  ...
  async function acceptWorkshopLogin(data, poll) {
    ...
    persistWorkshopIdentity(workshopIdentity); // 3.9.6 r64 #3：{name,avatar} 持久化，跨浏览器会话保署名与头像
    let auth = null;
    ...
    // r64：存储写入失败时诚实提示降级范围，不再伪装成「token 无效」。
    if (workshopTokenStorageBroken) toast('warn', '浏览器本地存储不可用：登录仅本次会话有效，刷新或重开酒馆后需重新登录');
    return { ...workshopAuth };
  }
  ...
  async function publishPackage(input, options = {}) {
    ...
    // 3.7.1 多源取值 → 3.9.6 r64 #3 持久化：内存 → localStorage（跨会话），任一有值即可署名。
  ...
  // 3.7.5 #6：作者永久删除自己的包（POST /:id/purge；区别于 DELETE=撤回）。乐观锁走 x-package-revision。
  async function deletePackagePermanent(id, revision) {
    if (!workshopAuth.loggedIn) throw new Error('请先完成 Discord 登录');
    if (!Number.isInteger(Number(revision)) || Number(revision) < 1) throw new Error('删除前必须刷新并取得有效 revision');
    const requestToken = getWorkshopToken();
    const res = await fetch(gatewayBaseUrl() + '/api/workshop/packages/' + encodeURIComponent(id) + '/purge', { method:'POST', credentials:'omit', headers:authHeaders({ 'x-package-revision':String(revision) }) });
    return workshopResponse(res, '删除工坊包', requestToken);
  }
  ...
  function packageToWorldbookText(pkg) {
    ...
    // 3.7.5 #5：普通世界因子包（非开局正文）直接取因子内容，安装成可读设定而非 JSON dump
    if (pkg?.type === 'world_factor' && pkg?.payload?.target !== OPENING_PACKAGE_TARGET && Array.isArray(pkg?.payload?.worldFactors) && pkg.payload.worldFactors[0]) {
      const wf = pkg.payload.worldFactors[0];
      const wfT = String(wf.title || pkg.title || '').trim();
      const wfC = String(wf.content || '').trim();
      if (wfC || wfT) return wfT && wfC ? (wfT + '：' + wfC) : (wfC || wfT);
    }
    return [
      '# ' + pkg.title,
      '',
  ...
  // 缺失 selected_extensions（未过开局页）默认全 3 条，保证商店内容不因跳过向导而丢失
  function extensionContent(draft = readOpeningDraft()) {
    const lines = [];
    const selected = Array.isArray(draft.selected_extensions) ? draft.selected_extensions : EXTENSION_PRESET_KEYS.slice();
    selected.forEach(key => { if (EXTENSION_PRESETS[key]) lines.push(EXTENSION_PRESETS[key]); });
    if (Array.isArray(draft.custom_extensions)) draft.custom_extensions.forEach(f => {
      const title = String((f && f.title) || '').trim();
      const content = String((f && f.content) || '').trim();
      if (content) lines.push(title ? title + '：' + content : content);
      else if (title) lines.push(title);
    });
    return lines.join('\n\n').trim();
  }
  // 影响预览用：只返回短标签（预设名 + 自定义标题），避免完整 shop 长文本刷屏
  function extensionLabels(draft = readOpeningDraft()) {
    const labels = Array.isArray(draft.selected_extensions) ? draft.selected_extensions.slice() : EXTENSION_PRESET_KEYS.slice();
    if (Array.isArray(draft.custom_extensions)) draft.custom_extensions.forEach(f => {
      const title = String((f && f.title) || '').trim();
  ...
  function openingWorldbookPayload(draft = readOpeningDraft()) {
    ...
    return {
      ...
      extension: extensionContent(draft),
  ...
  const sharedWorldbookManager = (() => {
    ...
    function isOpeningManaged(entry, tags, scheme) {
      ...
      return src === tags.opening && (name === scheme.identity || name === scheme.worldFactor || name === scheme.extension || kind === KIND.IDENTITY || kind === KIND.WORLD_FACTOR || kind === KIND.EXTENSION);
    }
    ...
    function createWorldbookManager(config = {}) {
      ...
      function buildCanonical(payload) {
        ...
        if (payload.extension) opening.push(makeConstantCanonical(scheme.extension, payload.extension, KIND.EXTENSION));
      ...
      function managedDelete(scope, entry) {
        ...
        // 3.7.5 F3：kind 级删除域 —— 只清「世界因子/功能拓展」两条内容条目，**绝不碰身份**。
        // 病灶：'opening' 域会连身份一起删，所以「注入创意工坊内容」不敢用它 → 玩家取消勾选全部世界因子后，
        // 旧的 [世界因子]当前设定 永久残留且 enabled，AI 继续吃到已被取消的因子，UI 显示为空、世界书说另一套。
        // 而同一个按钮里取消工坊包却会被删 —— 语义自相矛盾。有了这个域，两边统一为「取消即移除」。
        // 注意 isOpeningManaged 也认身份（name===scheme.identity / kind===IDENTITY），故必须再按 kind/name 收窄。
        if (scope === 'opening:content') {
          if (!isOpeningManaged(entry, tags, scheme)) return false;
          const kind = entry?.extra?.kind ?? entry?.meta?.kind;
          const name = entry?.name;
          return kind === KIND.WORLD_FACTOR || kind === KIND.EXTENSION
              || name === scheme.worldFactor || name === scheme.extension;
        }
        return false;
      }
      ...
      // 3.6.1 #5：新增 scope 'workshop+content'。
      // 病灶：buildCanonical 把 identity / worldFactor / extension **三者同归 opening 桶**，而
      // 「注入创意工坊内容」按钮传 scope:'workshop' → 只取 workshop 桶 → 它在代码上**永远写不出**
      // [世界因子]当前设定 / [功能拓展]当前设定；那两条只有「开始回忆」(scope:'identity') 会写。
      // 两个按钮因此严格互补、各写一半，与玩家预期（勾了世界因子→点注入→生效）不符。
      // 'workshop+content' = 工坊包 + 这两条内容，但**不含身份**（按钮语义明确不写身份）。
      // ⚠️ 为何不直接用 'all'：① 'all' 会把身份一并写入，越过按钮语义；② 'all' 的 deleteScopes 是
      // ['opening','workshop']，当身份被 Persona 抑制（payload.identity 为空 → 不进 upserts）时，
      // 已存在的身份条目会被 deleteScopes 删掉却无人重建 = 误删。
      // 本 scope 改用 kind 级删除域 'opening:content'（3.7.5 F3）：只清世界因子/功能拓展、绝不碰身份，
      // 于是「取消勾选 → 点注入 → 旧内容被移除」与工坊包侧的语义终于一致（详见 deleteScopes 处注释）。
      const CONTENT_KINDS = [KIND.WORLD_FACTOR, KIND.EXTENSION];
      function contentCanonicals(opening) { return opening.filter(c => CONTENT_KINDS.includes(c?.meta?.kind)); }
      function scopedCanonicals(scope, opening, workshop) {
        if (scope === 'identity') return opening.slice();
        if (scope === 'workshop') return workshop.slice();
        if (scope === 'workshop+content') return [...contentCanonicals(opening), ...workshop];
        return [...opening, ...workshop];
      }
      async function write(payload = {}) {
        const scope = payload.scope || 'all';
        const { opening, workshop } = buildCanonical(payload);
        const toValidate = scopedCanonicals(scope, opening, workshop);
        ...
        // 'workshop+content' 的空态**不早退**（3.7.5 F3）：两桶皆空恰恰是「玩家取消了全部勾选」的情形，
        // 此时正需要 deleteScopes:['opening:content'] 把世界书里残留的旧世界因子/功能拓展清掉 —— 早退等于
        // 让 AI 继续吃到已取消的内容。只把语义如实标成「移除」，由调用方 toast 成 info 而非「注入成功」。
        // （注：这条实际很少触发 —— extensionContent() 在 selected_extensions 缺失时默认返回全部 3 条预设，
        //   content 桶通常非空；审查 F2 曾指出用合并后的 toValidate.length 当总闸恒不可达，故按桶分别判断。）
        if (scope === 'workshop+content' && !workshop.length && !contentCanonicals(opening).length) {
          summary.warning = '未勾选任何世界因子 / 功能拓展 / 工坊内容 —— 已移除世界书中对应的已注入条目。';
        }
        const upserts = toValidate.slice();
        if (scope !== 'identity' && workshop.length) upserts.push(boundaryCanonical('start'), boundaryCanonical('end'));
        // 'workshop+content' 的删除域分两半，各有讲究：
        // ① 'workshop' —— **仅在工坊桶非空时**加入（审查 F1 · 破坏性回归）：否则「启用包A→注入→取消勾选A→
        //    再点注入」会走到：deleteScopes 删掉 A 与两条工坊边界，而 upserts 因 workshop.length===0 跳过
        //    boundary push → 整个工坊块静默消失，却仍 applied=true 报「注入成功」。旧 scope:'workshop' 在这种
        //    情况下是「早退+警告、一条不动」，这层保险不能因为新增 content 桶而丢掉。
        // ② 'opening:content' —— **恒定加入**（3.7.5 F3）：先清旧的世界因子/功能拓展条目，再由 upserts 写回
        //    当前勾选。玩家取消全部勾选时 = 只删不写 → 旧内容真正被移除，AI 不再吃到已取消的因子。
        //    这个域按 kind 收窄、不碰身份，所以不会重蹈 'all' 的 deleteScopes 误删身份的覆辙。
        const deleteScopes = scope === 'identity' ? ['opening']
                           : scope === 'workshop' ? ['workshop']
                           : scope === 'workshop+content' ? (workshop.length ? ['workshop', 'opening:content'] : ['opening:content'])
                           : ['opening','workshop'];
  ...
  const WORLDBOOK_FACTORY_BASELINE_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6/worldbook-factory-baseline.json?v=p9-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.9.6/worldbook-factory-baseline.json?v=p9-r1',
  ];
  const WORLDBOOK_FACTORY_BASELINE_SHA256 = 'ea9c669da37684e6c4880ff54aa6327c3432e1d6bead6673f1e9b75848aa8709';
    ...
    worldbookFactoryBaselinePromise = (async () => {
      ...
      for (const url of WORLDBOOK_FACTORY_BASELINE_URLS) {
        try {
          ...
          if (data?.schema !== 'worldbook-factory-baseline-v1' || data?.cardVersion !== VERSION || data?.sha256 !== WORLDBOOK_FACTORY_BASELINE_SHA256 || data?.counts?.core !== 34 || data?.counts?.variable !== 5 || data?.counts?.total !== 39) throw new Error('3.4.9 出厂基线契约或固定 SHA-256 不匹配');
          return data;
        } catch (error) { lastError = error; }
      }
      throw lastError || new Error('无法加载 3.4.9 世界书出厂基线');
  ...
  const WORLDBOOK_EDITOR_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6/worldbook-editor.js?v=p7-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.9.6/worldbook-editor.js?v=p7-r1',
  ];
  ...
  async function previewApplyOpeningPackage(pkg) {
    ...
    if (pkg.cardScope !== OPENING_PACKAGE_SCOPE || pkg.payload?.target !== OPENING_PACKAGE_TARGET) throw new Error('该包不是 3.4.9 开局正文模板');
    const inspected = await inspectWorkshopPackage(pkg);
  ...
  function previewOpeningWrites(draft = readOpeningDraft()) {
    ...
    worldbookManager.preview({
      scope:'all', identity:payload.identity, worldFactor:payload.worldFactor, extension:payload.extension,
      workshopEntries:payload.workshopEntries.map(item => ({ ...item, packageTarget:item.packageTarget || item.packageType })),
    });
    return { identity:payload.identity, worldFactor:payload.worldFactor, extension:payload.extension, workshopEntries:payload.workshopEntries.map(item => item.comment) };
  }
  ...
  async function rerollCurrentVariables() {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.9 的当前楼变量重算器，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
  ...
  async function previewVariableCorrection(instruction, floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.9 的变量定点修正器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function rerollFromAnalysis(analysisText, floorId) {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.9 的当前楼变量重算器，依据玩家给定的变量预分析与正文，只输出一个内含 <analysis> 和 <JSONPatch> 的 <UpdateVariable> 块，不生成正文。' },
        { role: 'user', content: prompt },
      ],
  ...
  async function repairVariableFormatViaLLM(floorId = 'latest') {
    ...
    const raw = normalizeGeneratedUpdateBlock(String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.9 的变量格式修复器，只输出一个内含 <analysis> 和 <JSONPatch> 的最小 <UpdateVariable> 块，只修格式不改语义。' },
        { role: 'user', content: prompt },
      ],
  ...
  // 3.4.9：拖尾灵动感调优——tailRatio 0.58→0.70（更多粒子参与甩尾）、dragShear 1.28→1.72（甩尾幅度更明显）
  // 3.4.9：radius 0→56（固定粒子场半径=56 buffer=28 CSS，球视觉仍 56px）；canvas 缓冲放大到 200（下方 ensureSidebarBall），
  ...
  class PetOrbRenderer {
    ...
    nudge(dx, dy) {
      ...
      // 3.4.9-r37：拖动=部分点阵本身被惯性留在后方(靠点阵连线拉成拖影)，非给每个点加尾巴线。位移上限放到 2.4r 配合 240px 画布不裁；甩离幅度走 dragShear，速度踢随幅度缩放
    ...
    _update(dt) {
      ...
      const dtFrac = dt / 16;
      // 3.4.9：松手能量衰减 0.86→0.90，尾巴归位更绵长不戛然而止（更灵动）
      ...
      for (const p of this._particles) {
        ...
        if (p.tail && (p.sx || p.sy || p.svx || p.svy)) {
          // ⑧c 欠阻尼弹簧：拉回中心偏移。3.4.9-r37 再软化——松手刚度 x1.7→x1.5 回中更绵，
    ...
    _draw() {
      ...
      // 3.4.9：绘制缩放系数改按粒子场半径（drawScale=2*r/56，等于旧 canvas.width/56 的值），
      ...
      ctx.restore();
      // 3.4.9-r37：删除错误的"拖尾线"绘制——数据史莱姆的甩尾=部分点阵本身被惯性留在后方(rx/ry 含 sx/sy 滞后偏移)+下方点阵连线自然拉成拖影，不再给每个点画尾巴线段。
      ctx.save();
  ...
  function calcRadialPositions(center, vw, vh) {
    ...
    const rArc = 122;    // 3.4.9：5 键沿弧分布，半径从 108 加大到 122 避免相邻键堆叠（≥28/|cos75°| 仍满足不与球重叠）
  ...
  function ensureHudAvatarMgrStyle(doc) {
    ...
    style.textContent = [
      // 3.4.9：overlay 用 left/top:0 + JS 显式设视口宽高（inset:0 在 ST 顶层 transform 下会塌成 32px 高→panel 跑屏外）；panel 用 calc(100%-…) 相对 overlay 硬边界防溢出
  ...
  // 3.4.9：HUD 段自含采集/缩略图，不跨闭包依赖开局段的 collectDialogSpeakers/avatarManagerThumb（那些闭包 root，HUD 段不可达会 ReferenceError）。
  ...
  // 3.4.9：HUD 段可达的 CC api getter——controlCenter() 在 bindOpeningPage 闭包内、HUD 段调它会 ReferenceError。
  ...
  // 3.4.9：显式把 overlay 尺寸钉到真实视口（绕 ST 顶层 transform 对 fixed inset:0 的高度塌缩劫持）
  ...
  function openHudAvatarManager() {
    ...
    sizeHudAvatarMgrOverlay(); // 3.4.9：钉视口尺寸→panel 正中且不超边界
    const win = hostWindow();
  ...
  function openSidebarMenu() {
    ...
    menu.addEventListener('click', (event) => {
      ...
      try {
        ...
        if (act === 'avatar') openHudAvatarManager(); // 3.4.9：轮盘第5键——聊天上下文气泡头像管理器
  ...
  const HUD_RT_BASE = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6';
  const HUD_RT_BASE_CF = 'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6';
  let hudPanel = null;
  let hudDrawer = null;
  const hudSession = {
    phase: 'idle',
  ...
  function publishHudBridge() {
    ...
    hudBridge = {
      ...
      // 3.6.1 #8a/#8e：真身跑在非 TH 托管的 blob iframe 里，全文对 persona/宏替换零接触，ST 的消息
      // 渲染管线不经过它 → {{user}} 永远是字面文本（标题显示 "{{user}}状态"），且把字面串当人名喂给
      // 头像兜底后，首字符取到 '{'、名字 hash 算出 hsl(54…) 土黄底 = 那块土黄色的 { 占位。
      // 不走 fns 快照（6920 注释记过 getMvu 同款坑：面板早于 TavernHelper 就绪时快照为 undefined），
      // 改为动态调用：CC 在顶层，resolveCurrentPlayerName 内建 name1 → 4 候选×2 window 的
      // substitudeMacros（注意 ST 官方就是这个少一个 t 的拼写）→ '玩家' 保底。
      // 只暴露 resolveCurrentPlayerName：真身当前仅需人设名本身；不搭 resolvePlayerText（无消费者＝
      // 白白扩大跨 iframe API 面，审查 F6）。将来真身若需要整段文本替换，再按需加。
  ...
  function ensureSidebar() {
    ...
    canvas.width = 480; canvas.height = 480; // 3.4.9-r37：2x 物理分辨率(240px CSS)，远大于 56px 球，四周留足空间让被甩离的点阵不被裁
  ...
  async function generateNpcPerspective(targetName) {
    ...
    const result = String(await generateRaw({
      ...
      max_chat_history: 0,
      ordered_prompts: [
        { role: 'system', content: '你是星月 3.4.9 的楼层内临时旁观视角生成器。结果只供玩家娱乐阅读，不进入后续上下文。' },
        { role: 'user', content: prompt },
      ],
    }) || '').trim();
  ...
  function closeActiveOpeningModal() {
    ...
    if (modal.matches?.('[data-xy-extension-modal]') && extensionEditorDirty(modal) && !hostWindow().confirm?.('功能拓展表单有未添加内容。确认放弃并关闭？')) return false;
  ...
  function renderOpeningDayEditor(draft = readDraft()) {
    ...
    try {
      ...
      if (statusNode) {
        ...
        statusNode.textContent = '正文有效 · ' + validation.bytes + ' bytes · ' + (dirty ? '已按当前聊天保存修改' : '正在使用 3.4.9 出厂版');
      }
  ...
  function extensionEditorDirty(modal = root.querySelector('[data-xy-extension-modal]')) {
    return [...modal?.querySelectorAll?.('[data-xy-ext-field]') || []].some(input => String(input.value || '').trim());
  }
  ...
  // 3.4.9 #3：包→角色草稿（characterDraftAsPackage 的反向），供「我的发布」编辑回填。字段一一对应，别复杂化。
  ...
  function setStep(next, options = {}) {
    ...
    root.querySelectorAll('.xy-step').forEach(button => {
      ...
      // 3.4.9：走过的步骤给完成态标记（✓ 表示访问过，不代表填写完整）
  ...
  function updateWorkshopStatusPills() {
    ...
    // 3.9.6 r64 #3：署名输入框回填持久化值（正在输入时不覆盖）
      ...
      detail.textContent = !parsed.found
        ...
        : (!parsed.valid ? ('结构块无效：' + (parsed.errors?.[0]?.message || '无法解析'))
          : (parsed.multiple ? '检测到多个星月结构块；同步时可合并为一个规范块。'
            : ('结构块有效 · 年级 ' + parsed.gradeLabel + (snapshot.position === PERSONA_DESCRIPTION_DISABLED_POSITION ? ' · 当前注入已禁用' : ''))));
    }
  ...
  function renderEnableLists() {
    ...
    root.querySelectorAll('[data-xy-enable-list]').forEach(list => {
      ...
      list.innerHTML = items.map(pkg => {
        ...
        // 3.9.6 r64 #1/#4b：角色卡片补「编辑/删除」（真机反馈「角色怎么删」——此前只有启用勾选；
    ...
    root.querySelectorAll('[data-xy-enabled-preview]').forEach(node => {
      ...
      // 3.4.9 功能拓展：勾选的商店板块 + 自定义拓展 → 预览用短标签（避免完整 shop 长文本刷屏）
      if (group === 'extension') {
        extensionLabels(draft).forEach(label => chips.push({ label }));
      }
      // 已启用的包 → 每个一个卡片
  ...
  // 3.4.9 功能拓展：自定义拓展列表（镜像 renderWorldFactorList）
  function renderExtensionList() {
    const box = root.querySelector('[data-xy-extension-list]');
    if (!box) return;
    const draft = readOpeningDraft();
    const list = Array.isArray(draft.custom_extensions) ? draft.custom_extensions : [];
    if (!list.length) { box.innerHTML = ''; return; }
    box.innerHTML = list.map((f, i) => {
      const title = escapeHtml(String((f && f.title) || '未命名拓展'));
      const content = escapeHtml(String((f && f.content) || ''));
      return '<div class="xy-card xy-wf-card"><div class="xy-wf-text"><strong>' + title + '</strong>' + (content ? '<span class="xy-muted">' + content + '</span>' : '') + '</div><button type="button" data-xy-opening-action="remove-extension" data-index="' + i + '">删除</button></div>';
    }).join('');
  }
  ...
  function writePreview(draft) {
    const cc = controlCenter();
    const preview = cc?.previewOpeningWrites ? cc.previewOpeningWrites(draft) : { identity: '', worldFactor: '', extension: '', workshopEntries: [] };
    const lines = [
      '写入目标：角色卡绑定世界书',
      '写入边界：--/星月工坊开始 与 --/星月工坊结束 之间',
      '写入方式：只整理开局与工坊条目，角色状态仍由正文与变量系统自然推进',
      '',
      // 3.6.1 #5（审查 F4）：标注各条目由哪个按钮写。「开始回忆」写身份+世界因子+功能拓展；
      // 「注入创意工坊内容」写世界因子+功能拓展+工坊包，**不写身份** → 不标注的话，玩家读到预览
      // 说要写身份、点注入却没写，是新的困惑源。
      '[星月开局]{{user}}身份设定　※仅「开始回忆」写入',
      preview.identity || '（沿用角色卡默认身份）',
      '',
      '[世界因子]当前设定',
      preview.worldFactor || '无',
      '',
      // 3.6.1 #5：补列功能拓展。previewOpeningWrites 一直有返回 extension（见其 return），
      // 只是这里从未显示 → 玩家在预览里看不到它会被写入。注入按钮改走 workshop+content 后更须如实列出。
      '[功能拓展]当前设定',
      preview.extension || '无',
    ];
  ...
  function renderWorkshop() {
    ...
    // 3.4.9 #2：有内容时隐藏提示语（提示语只在空态作引导）。
    const wsPrivacy = root.querySelector('[data-xy-workshop-privacy]');
    if (wsPrivacy) wsPrivacy.hidden = items.length > 0;
    ...
    grid.innerHTML = items.map(pkg => {
      ...
      const purgeUi = pendingActionUi('purge-package', pkg.id, '删除', '删除中…');
      // 3.7.5 #6：作者永久删除按钮（二次确认在 handler 里），给所有自己的包，用于清理堆积的旧包
      const purgeBtn = '<button type="button" class="xy-danger" data-xy-opening-action="purge-package" data-id="' + escapeHtml(pkg.id) + '" data-revision="' + escapeHtml(pkg.revision || '') + '"' + purgeUi.attrs + '>' + purgeUi.label + '</button>';
  ...
  root.addEventListener('input', event => {
    ...
    // 3.9.6 r64 #3：署名输入框逐键持久化（不等 blur——用户改完直接点发布也生效）
  ...
  function syncOpeningPendingButtons(pendingKey, pending) {
    const busyLabels = {
      ...
      'publish-world-factor':'发布中…', 'publish-extension':'发布中…', 'purge-package':'删除中…',
    };
  ...
  function openExtensionEditor(prefill) {
    ...
    const c = modal.querySelector('[data-xy-ext-field="content"]');
    if (t) t.value = prefill?.title || '';
    if (c) c.value = prefill?.content || '';
    modal.hidden = false;
    try { (t || c)?.focus(); } catch (_) {}
  }
  // 3.9.6 r64 #1：角色编辑器统一开启函数，镜像 openWorldFactorEditor 的「显式赋值」模式。
  ...
  root.addEventListener('click', async event => {
    ...
    if (choice && root.contains(choice)) {
      ...
      // 3.9.6 r64 #1：角色拓展"+"新建槽（世界因子同款气泡，取代旧「新建角色」按钮）→ 显式空白角色编辑器
      ...
      if (checkGroup) {
        // 3.4.9 #4c/#4d：点勾选框切换选中 / 点其他区域开编辑浮窗（预填该条内容，编辑后另存为自定义）
        if (choice.classList.contains('xy-wf-choice') && !event.target.closest('.xy-wf-check')) {
          if (checkGroup.dataset.xyCheckGroup === 'selected_extensions') {
            openExtensionEditor({
              title: choice.dataset.xyCheckValue || '',
              content: EXTENSION_PRESETS[choice.dataset.xyCheckValue] || '',
            });
          } else {
    ...
    const ASYNC_OPENING_ACTIONS = new Set(['refresh-workshop','publish-current-package','publish-character-package','publish-identity-template','publish-world-factor','publish-extension','vote-package','download-package','show-package-detail','withdraw-package','purge-package','edit-my-package','install-selected-package','uninstall-selected-package','apply-selected-opening-package','import-avatar-local','use-avatar-url','import-char-media','start-recall','export-character-package','avatar-manager-add','avatar-manager-import']);
    ...
    try {
      ...
      if (action === 'paste-char-media-url') {
        ...
        const overlay = doc.createElement('div');
        // r53 居中方案：overlay overflow:auto + panel margin:auto → 放得下两轴居中、放不下贴顶可滚，杜绝「居中溢出顶飞」
        overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.6);display:flex;overflow:auto;padding:16px;';
        const panel = doc.createElement('div');
        panel.style.cssText = 'margin:auto;background:#171310;border:1px solid rgba(224,178,123,.45);padding:14px;max-width:min(480px,94vw);width:100%;max-height:calc(100vh - 32px);overflow:auto;display:grid;gap:10px;color:#e6d7c3;font-size:13px;';
      ...
      if (action === 'publish-world-factor') {
        // 3.7.5 #5：世界因子编辑浮窗内发布——把当前填写的因子作为 world_factor 包发到创意工坊
        const cc = controlCenter();
        if (!cc?.publishPackage) throw new Error('控制中心发布 API 未就绪');
        const modal = root.querySelector('[data-xy-world-factor-modal]');
      ...
      // 3.4.9 功能拓展：编辑浮窗 open/close/add/remove（镜像世界因子）
      if (action === 'open-extension-editor') {
        openExtensionEditor();
      }
      if (action === 'close-extension-editor') {
        const modal = root.querySelector('[data-xy-extension-modal]');
        if (modal && extensionEditorDirty(modal) && !hostWindow().confirm?.('功能拓展表单有未添加内容。确认放弃并关闭？')) return;
        if (modal) modal.hidden = true;
      }
      if (action === 'add-extension') {
        ...
        if (!title && !content) throw new Error('请填写拓展名称或内容');
        const draft = readOpeningDraft();
        const list = Array.isArray(draft.custom_extensions) ? draft.custom_extensions.slice() : [];
        list.push({ title, content });
        writeOpeningDraft({ custom_extensions: list });
        renderEnableLists();
        if (modal) modal.hidden = true;
        toast('success', '已添加功能拓展：' + (title || content).slice(0, 20));
      }
      if (action === 'publish-extension') {
        // 3.7.5 #5：功能拓展编辑浮窗内发布——按所选分类作为扩展包发到创意工坊；payload 走 worldbook.content 干净路径
        const cc = controlCenter();
        if (!cc?.publishPackage) throw new Error('控制中心发布 API 未就绪');
        const modal = root.querySelector('[data-xy-extension-modal]');
        ...
        if (!title) throw new Error('发布功能拓展需要填写拓展名称');
      ...
      if (action === 'edit-local-character') {
        // 3.9.6 r64 #1：本地角色的正经再编辑入口（旧版唯一“再编辑”路径是新建按钮的脏草稿泄漏，泄漏修掉后必须补上）
      ...
      if (action === 'remove-enable-package') {
        // 3.9.6 r64 #4b：本地列表删除（真机反馈「角色怎么删」）。仅动本地草稿；云端工坊包在「我的发布」里撤回/删除。
        const key = String(button.dataset.key || '');
        const pkg = packages().find(item => packageIdentity(item) === key);
      ...
      if (action === 'purge-package') {
        // 3.7.5 #6：作者永久删除自己的包（双弹窗二次确认，防误删）
        const cc = controlCenter();
        if (!cc?.deletePackagePermanent) throw new Error('控制中心删除 API 未就绪');
        if (!confirm('永久删除这个工坊包？此操作不可撤销——云端包体、角色资产与投票都会被清除（本地草稿不受影响）。')) return;
        if (!confirm('再次确认：确定永久删除？删除后无法恢复。')) return;
        await cc.deletePackagePermanent(button.dataset.id, button.dataset.revision || undefined);
        state.publishSelection = null;
        toast('success', '已永久删除');
        await refreshWorkshop();
      }
      ...
      if (action === 'export-opening-draft') {
        downloadJson('xingyue-opening-draft-v3.4.9.json', readDraft());
      }
      ...
      if (action === 'restore-opening-source') {
        ...
        if (source) {
          ...
          if (!confirm('当前正文没有工坊来源。确认恢复 3.4.9 官方出厂版？')) return;
      ...
      if (action === 'restore-opening-official') {
        if (!confirm('确认恢复 3.4.9 出厂正文？当前聊天里的正文修改将被替换。')) return;
        restoreOfficialOpeningDay();
        applyDraftToFields();
        renderWizard();
        toast('success', '已恢复 3.4.9 出厂正文');
      }
      ...
      if (action === 'inject-workshop') {
        ...
        // 3.6.1 #5：scope 从 'workshop' 改为 'workshop+content'。原 'workshop' 只写 workshop 桶，
        // 而世界因子/功能拓展 tab 勾选的内容归在 opening 桶 → 本按钮从来注入不了它们（只有「开始回忆」会写），
        // 玩家勾了世界因子再点注入却没反应即源于此。新 scope 额外带上这两条内容，仍不写身份、不动 opening 域的删除。
        const draft = collectFields();
        const cc = controlCenter();
        let writeResult = null;
        try { writeResult = await cc?.writeOpeningWorldbookEntries?.(draft, { scope: 'workshop+content' }); } catch (error) { toast('error', error.message || String(error)); return; }
        const resultNode = root.querySelector('[data-xy-opening-result]');
        if (writeResult?.warning) toast('info', writeResult.warning);
        // 3.6.1 #5（审查 F1）：本按钮现在写的是「工坊内容 + 世界因子 + 功能拓展」，文案照实说；
        // 原文案只提"创意工坊内容"，在只写了世界因子/功能拓展时会误导。
        else if (writeResult?.applied) toast('success', '已注入世界书：工坊内容 / 世界因子 / 功能拓展');
  ...
  try {
    ...
    try { updateOpeningMediaUsage(root); } catch (_) {}
    return true;
  }
  // 3.9.6：媒体库用量映射进开局页 boot 面板（总监裁定）。数据来自 media_library.getUsage()
  // （已按「存档去重后」口径计量，即真实配额位置）；接口缺失（旧卡组件）→ 行保持隐藏，零影响。
  function updateOpeningMediaUsage(root) {
    const row = root?.querySelector?.('[data-xy-media-usage]');
    if (!row) return;
    const lib = mediaLibrary();
    if (typeof lib?.getUsage !== 'function') {
      if (!root.__xyMediaUsageRetried) {
        root.__xyMediaUsageRetried = true;
        scheduleOpeningTimer(() => { try { updateOpeningMediaUsage(root); } catch (_) {} }, 2500);
      }
      return;
    }
    let usage = null;
    try { usage = lib.getUsage(); } catch (_) { usage = null; }
    if (!usage || !Number.isFinite(usage.usedChars) || !Number.isFinite(usage.maxChars) || usage.maxChars <= 0) { row.hidden = true; return; }
    const fmt = chars => (chars / 1048576 >= 10 ? Math.round(chars / 1048576) : (chars / 1048576).toFixed(1)) + 'MB';
    const pct = Math.max(0, Math.round(usage.usedChars / usage.maxChars * 100));
    const tag = row.querySelector('[data-xy-media-usage-value]');
    const dot = row.querySelector('.xy-dep-dot');
    if (tag) {
      tag.textContent = fmt(usage.usedChars) + ' / ' + fmt(usage.maxChars) + '（' + pct + '%）';
      tag.classList.toggle('warn', pct >= 80);
    }
    if (dot) { dot.classList.toggle('warn', pct >= 80); dot.classList.toggle('ok', pct < 80); }
    row.hidden = false;
  }
  ...
  const OPENING_PAGE_REVISION = '20260718-396-offline-r69';
  // 3.7.5 #3：随 opening-page.html 的容器查询修复同步更新（改了远程 HTML 必须重算此 pin，
  // 否则 verifyOpeningPageHtml 会抛「远程开局页完整性校验失败」→ 玩家的开局页直接加载不出来）。
  const OPENING_PAGE_SHA256 = '1780735b43d96b1a54aa003ad3da36cd42b1ef29c4660308cb9acda3f46b2f15';
  // r64 源策略（Bug2 修复）：
  ...
  //   ② 工坊 VPS 镜像（腾讯云，大陆可达；nginx 反代缓存 raw@main，SHA 门兜住内容正确性，镜像未部署时快速 404 落下一源）；
  //   ③ raw@main 直源；仅当 loader 未写入 __xyRuntimeBase（极旧回落路径）才补 jsDelivr @main 双源。
  const OPENING_PAGE_SOURCES = (() => {
    ...
    try {
      ...
      if (/^https:\/\//.test(base)) {
        list.push(base + '/opening-page.html?v=' + OPENING_PAGE_REVISION);
        derived = true;
        const pin = base.match(/^https:\/\/cdn\.jsdelivr\.net\/(gh\/[^@]+@[0-9a-f]{7,40}\/.+)$/i);
        if (pin) list.push('https://testingcf.jsdelivr.net/' + pin[1] + '/opening-page.html?v=' + OPENING_PAGE_REVISION);
      }
    } catch (_) {}
    list.push(DEFAULT_GATEWAY_URL + '/gh-raw/main/runtime/xingyue/3.9.6/opening-page.html?v=' + OPENING_PAGE_REVISION);
    list.push('https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.9.6/opening-page.html?v=' + OPENING_PAGE_REVISION);
    if (!derived) {
      list.push(
        RUNTIME_BASE_URL + '/opening-page.html?v=' + OPENING_PAGE_REVISION,
        'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.9.6/opening-page.html?v=' + OPENING_PAGE_REVISION,
      );
    }
    return [...new Set(list)];
  })();
  // r64：6.5s→12s——大陆 4G 经国际线路拉 313KB 常超 6.5s，会把活源误判成死源；常态 4 源，最坏 ~48s 后必有结论（成功/兜底/报错）。
  ...
  function sanitizeOpeningPageHtml(html, doc) {
    ...
    const root = parsed.querySelector('[data-xy-opening-page="3.9.6"]');
    if (!root
  ...
  function failOpeningRemoteAttempt(mount, attempt, error) {
    ...
    // r64：全部远程源失败 → 先吃卡内兜底副本（3.9.6+ 卡 loader 注入 __xyOpeningFallbackHtml；旧卡无此全局，行为不变）。
    if (consumeOpeningEmbeddedFallback(mount)) return;
    markOpeningRemoteIdle(mount);
    // r64：按失败构成分文案——「校验未通过」和「连不上」的自救方式完全不同，混在一句里没法排查。
    const verifyFails = Number(attempt.verifyFails) || 0;
    const netFails = Number(attempt.netFails) || 0;
    let reason;
    if (verifyFails && !netFails) reason = '已连上更新源，但内容校验未通过（多为 CDN 缓存尚未刷新）。<br>稍等几分钟后点击下方按钮重试即可。';
    else if (verifyFails && netFails) reason = '部分来源无法连接，其余来源内容校验未通过（多为 CDN 缓存尚未刷新）。<br>请检查网络或稍后点击下方按钮重试。';
    else reason = '所有安全来源均无法连接。<br>请检查网络，恢复后点击下方按钮重试。';
    renderOpeningRemotePhase(mount, 'error', '开局界面加载失败。<br>' + reason);
  }
  function consumeOpeningEmbeddedFallback(mount) {
    let html = '';
    try { html = String(window.__xyOpeningFallbackHtml || ''); } catch (_) {}
    if (!html) return false;
    try {
      const doc = mount.ownerDocument || document;
      // 消毒+结构校验与远程路径完全一致；SHA 跳过——副本由卡内 loader 自带，与本 runtime 同信任域。
  ...
  async function tryOpeningRemoteSource(doc, mount, attempt, sourceIndex) {
    if (!isOpeningAttemptCurrent(mount, attempt)) return;
    const remaining = Number(attempt.deadlineAt || 0) - Date.now();
    if (sourceIndex >= OPENING_PAGE_SOURCES.length || remaining <= 0) {
    ...
    const sourceTimeoutMs = Math.max(1, Math.min(OPENING_PAGE_SOURCE_TIMEOUT_MS, remaining));
    ...
    try {
      ...
      // r64：失败分类计数（超时/网络归 netFails），供最终报错选对自救文案。
      if (/完整性校验失败|结构或版本不匹配/.test(String(error?.message || ''))) attempt.verifyFails = (attempt.verifyFails || 0) + 1;
      else attempt.netFails = (attempt.netFails || 0) + 1;
      cleanupOpeningAttemptSource(attempt, controller);
  ...
  function startOpeningRemoteLoad(mount, options = {}) {
    ...
    const loadedPage = mount.querySelector?.('[data-xy-opening-page="3.9.6"]')
    ...
    try {
      if (window.__xyRuntimeSource === 'embedded' && consumeOpeningEmbeddedFallback(mount)) return true;
    } catch (_) {}
    ...
    const attempt = {
      owner: runtimeOwner.id,
      controller: null,
      timeout: null,
      sourceToken: null,
      sourceIndex: 0,
      deadlineAt: Date.now() + OPENING_PAGE_TOTAL_TIMEOUT_MS,
      lastError: '',
      verifyFails: 0,
      netFails: 0,
    };
