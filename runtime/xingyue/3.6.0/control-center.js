(() => {
  ...
  const RUNTIME_BASE_URL = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0';
...
- 淫纹绘制器：可以在人体上附着带有发光效果（动情时互相发光）的生物适应性颜料（仅被绘制者直接和{{user}}可见），可通过再次使用设备移除 对人体无损；拥有两种模式，模式1是绘制阴道测量尺会扫描阴道的默认深度绘制在小腹上，模式2是淫纹模式 会根据角色的特点生成淫纹绘制在小腹上
  ...
  let workshopEditingPackageId = null; // 3.6.0 #4：正在「编辑更新」的工坊包 id（edit-my-package 设置、新建角色清空），锚定 id 使更新覆盖原包而非按名字现算 fork
  ...
  const GIT_RUNTIME_REVISION = '3.6.0-stability-r56-20260716';
  ...
  function validatePackage(pkg, allowedTypes) {
    ...
    if (openingByScope) {
      ...
      if (!['3.4.1', '3.4.2', '3.4.3', '3.4.4', '3.4.5', '3.4.9', '3.5.0', '3.6.0'].includes(pkg.payload.compatibility?.minRuntimeVersion)) throw new Error('开局正文包最低 runtime 必须为 3.4.1 至 3.6.0');
  ...
  // 3.6.0 #4：Discord 显示名做「会话级」持久化（sessionStorage：关标签即清、不入 localStorage、不发 gateway，隐私铁律不破），
  ...
  async function checkWorkshopAuth() {
    ...
    try {
      ...
      // 3.6.0 #4：已登录但内存无 identity（页面刷新/会话恢复后）→ 从会话级存储补回显示名，供发布署名
  ...
  async function acceptWorkshopLogin(data, poll) {
    ...
    persistWorkshopIdentityName(workshopIdentity?.name || ''); // 3.6.0 #4：会话级存显示名，供刷新/会话恢复后补署名
    let auth = null;
  ...
  async function logout() {
    ...
    commitWorkshopAuth({ checked:true, loggedIn:false, publisherId:'', error:'' });
    workshopIdentity = null;
    persistWorkshopIdentityName(''); // 3.6.0 #4：登出清会话级署名
    return { ...workshopAuth };
  }
  ...
  // 3.6.0 #6：作者永久删除自己的包（POST /:id/purge；区别于 DELETE=撤回）。乐观锁走 x-package-revision。
  ...
  function packageToWorldbookText(pkg) {
    ...
    // 3.6.0 #5：普通世界因子包（非开局正文）直接取因子内容，安装成可读设定而非 JSON dump
  ...
  const WORLDBOOK_FACTORY_BASELINE_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0/worldbook-factory-baseline.json?v=p9-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.6.0/worldbook-factory-baseline.json?v=p9-r1',
  ];
  ...
  const WORLDBOOK_EDITOR_URLS = [
    ...
    'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0/worldbook-editor.js?v=p7-r1',
    'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.6.0/worldbook-editor.js?v=p7-r1',
  ];
  ...
  const HUD_RT_BASE = 'https://cdn.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0';
  const HUD_RT_BASE_CF = 'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0';
  let hudPanel = null;
  let hudDrawer = null;
  const hudSession = {
    phase: 'idle',
  ...
  function characterDraftAsPackage() {
    ...
    return {
      type: 'character',
      // 3.6.0 #4：编辑更新时锚定原包 id（含改名也不 fork）；新建角色 workshopEditingPackageId=null → 按名字派生
  ...
  function renderWorkshop() {
    ...
    grid.innerHTML = items.map(pkg => {
      ...
      // 3.6.0 #6：作者永久删除按钮（二次确认在 handler 里），给所有自己的包，用于清理堆积的旧包
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      ...
      if (action === 'open-character-editor') {
        workshopEditingPackageId = null; // 3.6.0 #4：新建角色不绑定任何已发布包，发送时按名字新建（不会误覆盖上次「编辑」的包）
        const modal = root.querySelector('[data-xy-character-editor-modal]');
        if (modal) {
          const cd = readCharacterDraft();
      ...
      if (action === 'publish-world-factor') {
        // 3.6.0 #5：世界因子编辑浮窗内发布——把当前填写的因子作为 world_factor 包发到创意工坊
        const cc = controlCenter();
        if (!cc?.publishPackage) throw new Error('控制中心发布 API 未就绪');
        const modal = root.querySelector('[data-xy-world-factor-modal]');
        const title = textOf(modal?.querySelector('[data-xy-wf-field="title"]')?.value, '');
      ...
      if (action === 'publish-extension') {
        // 3.6.0 #5：功能拓展编辑浮窗内发布——按所选分类作为扩展包发到创意工坊；payload 走 worldbook.content 干净路径
        const cc = controlCenter();
        if (!cc?.publishPackage) throw new Error('控制中心发布 API 未就绪');
        const modal = root.querySelector('[data-xy-extension-modal]');
        const title = textOf(modal?.querySelector('[data-xy-ext-field="title"]')?.value, '');
      ...
      if (action === 'edit-my-package') {
        ...
        workshopEditingPackageId = String(detail.id || '') || null; // 3.6.0 #4：锚定原包 id，改完「发送到创意工坊」即覆盖更新（revision 由 publishPackage 自动探测取最新）
      ...
      if (action === 'purge-package') {
        // 3.6.0 #6：作者永久删除自己的包（双弹窗二次确认，防误删）
        const cc = controlCenter();
  ...
  const OPENING_PAGE_SHA256 = 'ab26ff5a596c2e163cc1bf1069355a0ae194fe9f772b5ae5d9f747083cf43714';
  const OPENING_PAGE_SOURCES = (() => {
    ...
    list.push(
      ...
      'https://testingcf.jsdelivr.net/gh/LiarMTTT/rolecard-diy-workshop@main/runtime/xingyue/3.6.0/opening-page.html?v=' + OPENING_PAGE_REVISION,
      'https://raw.githubusercontent.com/LiarMTTT/rolecard-diy-workshop/main/runtime/xingyue/3.6.0/opening-page.html?v=' + OPENING_PAGE_REVISION,
    );
    return list;
  })();
  ...
  function sanitizeOpeningPageHtml(html, doc) {
    ...
    const root = parsed.querySelector('[data-xy-opening-page="3.6.0"]');
    if (!root
  ...
  function startOpeningRemoteLoad(mount, options = {}) {
    ...
    const loadedPage = mount.querySelector?.('[data-xy-opening-page="3.6.0"]')
